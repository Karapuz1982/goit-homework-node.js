import Joi from "joi";
 ...
 export const contactAddSchema = Joi.object({
  ...
  email: Joi.string().email({ minDomainSegments: 2, tlds: { allow: ['com', 'net'] } }).messages({
    ...
    'string.minDomainSegments': 'The "email" field must have at least {#limit} domain segments',
    'string.tlds': 'The "email" field must have a valid top-level domain',
  }),
phone: Joi.string().pattern(/^\(\d{3}\) \d{3}-\d{4}$/).messages({
    'string.pattern.base': 'The "phone" field must match the format (123) 456-7890',
  })
});